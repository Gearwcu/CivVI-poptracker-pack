...
civilization_vi_location = {}
civilization_vi_location.__index = civilization_vi_location
...
function civilization_vi_location.new(name)
    local self = setmetatable({}, civilization_vi_location)
    if name then
...
function civilization_vi_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = civilization_vi_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function civilization_vi_location:connect_two_ways(exit, rule)
...
function civilization_vi_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function civilization_vi_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function civilization_vi_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function civilization_vi_location:accessibility()
...
-- 
function civilization_vi_location:discover(accessibility, keys)
...
entry_point = civilization_vi_location.new("entry_point")
